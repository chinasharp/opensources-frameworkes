...
public class StringHelper {
	...
	public static final Log logger = LogFactory.getLog(StringHelper.class);
	...
	/**
	 * 将字符串对象按给定的分隔符separator转象为Long数组对象
	 *
	 * @param str
	 * @param separator
	 * @return
	...
	public static Long[] strToLongArray(String str, String separator) {
		StringTokenizer strTokens = new StringTokenizer(str, separator);
		Long[] array = new Long[strTokens.countTokens()];
		int i = 0;
		while (strTokens.hasMoreTokens()) {
			array[i] = Long.valueOf(strTokens.nextToken().trim());
			i++;
		}
		return array;
	}
		...
		for (int i = 0; i < strs.length; i++) {
			if ("".equals(strs[i])) {
				...
				if (strs[i].compareTo("a") >= 0) {
					...
					inputString = inputString.replaceAll("\\\\".concat(strs[i]), "");
				}
			}
		}
	...
	public static String fillStr(String inStr, String fill, int length, int direction) {
		...
		if (direction == AFTER) {
			return tempStr + inStr;
		} else {
			return inStr + tempStr;
		}
	}
	 ...
	 * @return
	 */
	public static short strToShort(String numStr) {
		try {
			if (isNullOrBlank(numStr)) {
				return 0;
			}
			...
			logger.error("strToShort failed:", e);
		}
		return 0;
	}
	...
	public static double strToDouble(Object o) {
		double rs = 0;
		try {
			if (o == null || isNullOrBlank(o.toString())) {
				return 0;
			}
			...
			if (paramters != null) {
				for (String s : paramters) {
					if (s.startsWith(prefix)) {
						String[] paramTemp = s.split("=");
						if (paramTemp.length > 1) {
							returnParam = paramTemp[1];
						}
					}
				}
			}
	...
	public static long yuan2Cent(String src) {
		...
		long ret = 0;
		try {
			ret = Math.round(Double.parseDouble(src) * 100);
		} catch (Exception e) {
		}
	...
	public static String cent2Yuan(long src, boolean floor) {
		DecimalFormat sdfYuan = new DecimalFormat("0.00");
		String yuan = "0.00";
		try {
			yuan = sdfYuan.format(src / 100.00);
		} catch (Exception e) {
		}
	 ...
	 * @param <V>
	 * @param src               源字符串
	 * @param keyClass          生成的Map的Key的类型，默认String
	 * @param valueClass        生成的Map的Value的类型，默认String
	 * @param fieldDelimiter    字段与字段之间的分隔符，默认&
	 * @param keyValueDelimiter key和value之间的分隔符，默认=
	 * @return
	 */
	 ...
	 * @param map
	 * @param head        输出的头
	 * @param entryPrefix 每一项输出的前缀
	 * @param foot        输出的脚
	 * @param isOneItemPl 是否每行一项
	 * @param kvSep       Key和Value的分隔符
	 * @return
	 */
	public static String map2String(Map map, String head, String entryPrefix, String foot, boolean isOneItemPl,
		...
		for (Iterator iterator = map.entrySet().iterator(); iterator.hasNext(); ) {
			...
			buff.append(entryPrefix).append(entry.getKey()).append(kvSep).append(StringHelper.ArrayToString((Object[]) entry.getValue()));
	...
	public static boolean isLetterOrNumber(String src) {
		...
		try {
			byte[] bytesOfSrc = src.getBytes(StandardCharsets.UTF_8);
