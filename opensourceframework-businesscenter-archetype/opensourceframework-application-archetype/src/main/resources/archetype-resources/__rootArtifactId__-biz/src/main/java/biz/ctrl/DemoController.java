...
import ${groupId}.center.${dependency-center-name}.api.IDemoApi;
...
public class DemoController {
	...
	@DubboReference(group = "${symbol_dollar}{provider.${dependency-center-name}.service.group}" , version = "${symbol_dollar}{provider.${dependency-center-name}.service.version}" , protocol = "${symbol_dollar}{provider.${dependency-center-name}.service.protocol}")
	...
	public RestResponse<DemoRespDto> loadBalancedSave(@RequestBody DemoReqDto demoUserReqDto){
		...
		ResponseEntity<Object> responseEntity =
				restTemplate.postForEntity("http://opensourceframework-${dependency-center-name}-center/v1/op/saveOrUpdate",httpEntity , Object.class);
	...
	public RestResponse<DemoRespDto> dubboSave(@RequestBody DemoReqDto demoUserReqDto){
		return demoApi.saveOrUpdate(demoUserReqDto);
	}
